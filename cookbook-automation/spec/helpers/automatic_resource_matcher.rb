...
# original by Matt Clifton (lynx44)
# available at https://github.com/lynx44/chefspec_extensions

module ChefSpec
  # automatically creates matchers for all chef resources
  module AutomaticResourceMatcher
    ...
    def method_missing(meth, *args, &block)
      ...
      if resource_matcher_candidate?(method_name, args)
        ...
        cookbook_matches = find_cookbooks_with_matching_resources(
          cookbook_candidates, method_name)
    ...
    def cookbook_paths
      Chef::Config[:cookbook_path] if Chef::Config[:cookbook_path].is_a?(Array)
      [Chef::Config[:cookbook_path]] unless \
        Chef::Config[:cookbook_path].is_a?(Array)
    end
    ...
    def parse_lwrp(cookbook, method_name)
      method_name = "#{method_name}_default" if method_name.count('_') == 1
    ...
    def find_cookbooks_with_matching_resources(cookbook_candidates, method_name)
      ...
      cookbook_candidates.each do |cookbook|
        ...
        cookbook_matches.push(cookbook) if \
          cookbook_has_resource?(cookbook, resource_parts[:resource_name])
      end
      cookbook_matches
    end
    ...
    def create_matcher(args, cookbook, method_name)
      ...
      ChefSpec::Matchers::ResourceMatcher.new(
        ...
        resource_definition[:action].to_sym,
        args[0]
      )
    end
  end
