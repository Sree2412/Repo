...
public class ConceptualNearDupeRequest {
	APIFactory apiFunction = new APIFactory();
	final String mockPath = "\\\\10.12.235.31\\MPQATeam\\MockReferences\\ConceptualDupeCluster\\";
	final String matrixPath = "\\\\10.12.235.31\\MPQATeam\\MatrixReferences\\ConceptualNearDupeCluster\\"+"TestOutput_"+apiFunction.function.getDate()+".xlsx";
	...
	public void preRequisite() throws Exception
	{
		...
		apiFunction.function.setResultFile(matrixPath,"NearDupeClusterResults");
	}
	...
	{
		String actResult;
		String clusterID = "ConceptualDupeCluster-"+indexName;
		apiFunction.function.setUpLog4j(description);
		Log.startTestCase(description);
		apiFunction.environmentSetup(indexName,"no",DBName_MP,ConnectorName,testCaseID);
		apiFunction.createClusterConceptualNearDupe(indexName, clusterID);
		apiFunction.runClusterQuery(indexName,clusterID,"NearDupeCluster");
		actResult = apiFunction.getClusterResults(indexName,clusterID);
		String expResult = apiFunction.function.getJSONStringFromFile(mockPath+"ConceptualDupeCluster-"+indexName+".json");
		//apiFunction.function.copyResultsDirectly(mockPath+clusterID,actResult);
		//copyClusterResultsInFile(description);
