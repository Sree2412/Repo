...
# Directory
extracted_iso_directory = File.join(Chef::Config[:file_cache_path], 'msproject') # Location to extract the ISO

# Paths
iso_source_full_file_path = File.join(node['msproject']['source'], node['msproject']['professional']['filename']) # path to the source ISO file
iso_local_full_file_path = File.join(Chef::Config[:file_cache_path], node['msproject']['professional']['filename']) # path to where ISO should be dowloaded to
...
execute 'extract_msproject_iso' do
  command "7z.exe x -y -o#{extracted_iso_directory} #{iso_local_full_file_path}"
  # not_if { File.exists?(executable_full_file_path) }
end

# Create installation config file
template 'copy_silent_install_config_locally' do
  source 'Config-Project.erb'
  path config_full_file_path
  variables(
    pid_key: node['msproject']['pid_key'],
    auto_activate: node['msproject']['auto_activate']
  )
  action :create
...
package node['msproject']['professional']['package_name'] do # ~FC009
  ...
  returns [3010, 1641, 0]
  not_if { registry_key_exists?(node['msproject']['registrykey']['64bit'], :machine) }
end
